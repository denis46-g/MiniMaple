...
var _miniMaple = require("./miniMaple");
...
function diffPolynom() {
    ...
    const my_minimaple = new (0, _miniMaple.MiniMaple)("4*x^3 - x^-2", "-x");
    let mon = my_minimaple.GetArrayMononoms();
    document.getElementById("result_output").innerHTML = mon[1]; //parseFloat('-x^-2'.split('').reverse().join('')).toString().split('').reverse().join('');
}

},{"./miniMaple":"dDJfm"}],"dDJfm":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "MiniMaple", ()=>MiniMaple);
class MiniMaple {
    ...
    if(arr_coef.length == 0)
        ...
        return result;*/ if (this._polynom == undefined || this._polynom.replaceAll(" ", "").length == 0) return "non";
        if (this._diff_variable == undefined || this._diff_variable.replaceAll(" ", "").length != 1) return "Ошибка. Переменная, по которой дифференцируем, должна быть записана одной буквой.";
        ...
        for(let i = 0; i < mononoms.length; i++){
            ...
            if (mononoms[i][mononoms[i].length - deg.length - 1] == "-") deg = "-" + deg;
            let variable = mononoms[i].match(/[a-zA-Zа-яА-Я]/g);
            if (variable == null) {
                if (mononoms[i] != coef) return "Ошибка. Некорректно задан многочлен: свободный член должен быть числом.";
            } else if (variable.length > 1) return "Ошибка. Одночлен должен содержать максимум одну переменную.";
            else if (variable == this._diff_variable) {
                if (!isNaN(coef) && !isNaN(deg)) {
                    if (mononoms[i] != coef + "*" + variable + "^" + deg) return "Ошибка. Некорректно задан многочлен : одночлен с явными коэффициентом и степенью не соответствует шаблону.";
                    else {
                        if (res != "" && parseFloat(coef) * parseFloat(deg) > 0) res += " + ";
                        res += parseFloat(`${parseFloat(coef) * parseFloat(deg)}`) == 1 ? "" : `${parseFloat(coef) * parseFloat(deg) == -1 ? "- " : parseFloat(coef) * parseFloat(deg) + "*"}`;
                        if (deg - 1 == 1) res += variable + " ";
                        else res += variable + "^" + `${parseFloat(deg - 1) > 0 ? parseFloat(deg - 1) : "(" + parseFloat(deg - 1) + ") "}`;
                    }
                } else if (!isNaN(coef)) {
                    if (mononoms[i] != coef + "*" + variable) return "Ошибка. Некорректно задан многочлен : одночлен с явным коэффициентом не соответствует шаблону";
                    else if (coef > 0) {
                        if (res == "") res += coef + " ";
                        else res += "+ " + coef + " ";
                    } else if (res == "") res += "-" + parseFloat(-1 * coef) + " ";
                    else res += "- " + parseFloat(-1 * coef) + " ";
                } else if (!isNaN(deg)) {
                    if (mononoms[i][0] == "-" && mononoms[i] == "-" + variable + "^" + deg) {
                        if (res != "" && parseFloat(deg) < 0) res += " + ";
    ...
    /*Получение массива одночленов многочлена*/ GetArrayMononoms() {
        this._polynom = this._polynom.replaceAll(" ", "");
        this._polynom = this._polynom.replaceAll("-", "+-");
        if (this._polynom[0] == "+") this._polynom = this._polynom.substring(1);
        let arr = this._polynom.split("+");
        for(let i = 0; i < arr.length; i++)if (arr[i][arr[i].length - 1] == "^") {
            let first = arr[i];
            let second = arr[i + 1];
            arr.splice(i, i + 1);
